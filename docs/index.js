    ...
    (function() {
      ...
      function setDocument(node) {
        ...
        sortOrder = function(a, b) {
          ...
          var compare3 = !a.compareDocumentPosition - !b.compareDocumentPosition;
          if (compare3) {
            return compare3;
          }
          compare3 = (a.ownerDocument || a) == (b.ownerDocument || b) ? a.compareDocumentPosition(b) : 1;
          if (compare3 & 1 || !support.sortDetached && b.compareDocumentPosition(a) === compare3) {
...
var _window2 = window;
var telegram_default = _window2.Telegram.WebApp;
...
(function(ui) {
  function initialize() {
    import_jquery.default(document).on("scroll", function() {
  ...
  ui.header = {
    ...
    update: (board) => {
      ...
      if (board.isme) {
        ...
        import_jquery.default("#iuser").text(`bilft@${_username}`);
      }
    }
  };
})(ui || (ui = {}));
  ...
  ui.error = {
    show: (error2, autohide = true) => {
      ...
      if (error2 && typeof error2 === "string") {
        message = error2;
      } else if ("message" in error2 && typeof error2.message === "string") {
...
var ui;
(function(ui) {
  let _is_textarea_active = false;
  ui.send = {
    active: () => {
      return _is_textarea_active;
    },
    data: () => {
      return import_jquery.default("#send-data").serializeArray().reduce((previous, current) => {
        const data = {};
    ...
    enable: () => {
      import_jquery.default("#send-data").find("#textarea").fadeTo(0.12, 1);
      import_jquery.default("#send-data").find("#textarea").prop("disabled", false);
    },
    disaable: () => {
      import_jquery.default("#send-data").find("#textarea").fadeTo(0.12, 0.5);
      import_jquery.default("#send-data").find("#textarea").prop("disabled", true);
    },
    clear: () => {
      import_jquery.default("#send-data").find("#textarea").val("");
    },
    onactive: (callback) => {
      import_jquery.default("#send-data").find("#textarea").on("focus", () => {
        _is_textarea_active = true;
        callback(true);
      });
      import_jquery.default("#send-data").find("#textarea").on("blur", () => {
        _is_textarea_active = false;
        callback(false);
      });
    },
    onchange: (callabck) => {
      import_jquery.default("#send-data").find("#textarea").on("input propertychange", () => {
        callabck(ui.send.data().content);
      });
    }
  };
})(ui || (ui = {}));
  ...
  ui.footer = {
    text: (text) => {
      if (text && text.length > 0) {
        import_jquery.default("#footer").show();
        import_jquery.default("#footer").find("#alert").text(text);
      } else {
        import_jquery.default("#footer").hide();
        import_jquery.default("#footer").find("#alert").text("");
      }
    }
  };
})(ui || (ui = {}));
  ...
  ui.timeline = {
    prepend: (array) => {
      array.data.reverse().forEach((i) => {
        import_jquery.default("#timeline").prepend(_timeline.noteHTML(i));
      });
    },
    append: (array) => {
      array.data.forEach((i) => {
        import_jquery.default("#timeline").append(import_jquery.default(_timeline.noteHTML(i)));
      });
      ui.timeline.footer(array);
    },
    replace: (array) => {
      ui.timeline.clear();
      ui.timeline.append(array);
      ui.timeline.footer(array);
    },
    clear: () => {
      import_jquery.default("#timeline").empty();
    },
    footer: (array) => {
...
(function(ui) {
  ...
  ui.button = {
    update: (state) => {
      const _button = telegram_default.MainButton;
      switch (state) {
        case "send":
  ...
  ui.cursor = {
    onvisibleonce: (callback) => {
      const handler = function() {
        ...
        const elementTop = import_jquery.default(cursor2).offset().top;
        const elementBottom = elementTop + import_jquery.default(cursor2).outerHeight();
...
(function(api) {
  ...
  async function call(method, path, data3) {
    const authentication = {
      authentication_data: telegram_default.initData
    };
    return instance[method](path, { ...authentication, ...data3 }).then((result) => {
  ...
  api.board = {
    resolve: async (data3) => {
      return call("post", "/board/resolve", { value: data3.board });
    },
    write: async (data3) => {
      return call("post", `/board/createNote`, { board: data3.board, content: data3.content });
    },
    get: async (data3) => {
      return call("post", `/board/getNotes`, { board: data3.board, next: data3.next });
    }
  };
})(api || (api = {}));
var api_default = api;
...
// source/index.ts
var ready = function(board) {
  button.share(board);
  ui_default.send.onactive((active) => {
    const empty = ui_default.send.data().content.length == 0;
    if (active) {
      if (empty) {
        ui_default.button.update("hidden");
      } else {
        button.send(board);
      }
    } else {
  ...
  ui_default.send.onchange((content) => {
    if (content.length > 0) {
      button.send(board);
    } else if (ui_default.send.active()) {
      ui_default.button.update("hidden");
    } else {
      button.share(board);
    }
...
var load = function(board, next) {
  ui_default.header.loading(true);
  api_default.board.get({ board, next }).then((notes) => {
    ui_default.timeline.append(notes);
    const next2 = notes.next;
    if (!next2) {
      return;
    }
...
var button = {
  send: (board) => {
    ui_default.button.update("send");
    ui_default.button.click(() => {
      const content = ui_default.send.data().content;
      if (content.length == 0) {
        return;
      }
      ui_default.button.update("sending");
      ui_default.header.loading(true);
      ui_default.send.disaable();
      api_default.board.write({ board, content }).then((note) => {
        ui_default.send.clear();
        ui_default.timeline.prepend({ data: [note] });
        button.share(board);
      }).catch((error) => {
        ui_default.error.show(error);
        ui_default.button.update("send");
        ...
        ui_default.header.loading(false);
        ui_default.send.enable();
      });
    });
  },
  share: (board) => {
    switch (telegram_default.platform) {
      case "android":
      case "android_x":
      case "ios":
        break;
    ...
    ui_default.button.update("share");
    ui_default.button.click(() => {
      ...
      telegram_default.openTelegramLink(`http://t.me/share/url?url=${_url}&text=${_text}`);
    });
  }
};
var state = undefined;
